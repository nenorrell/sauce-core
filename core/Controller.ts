...
export class Controller{
    ...
    public async checkPolicies(){
        let policies = new Policies(this.config);
    ...
    private async validateReqBodyParam(schemaLevel :RouteParam, ancestor ?:RouteParam, obj ?:any) :Promise<any>{
        if(ancestor){
    ...
    private validateParamType(param :RouteParam, requestParam :any) :void{
        if(requestParam){
            if(!this.isValidTypes(param.type, requestParam)){
                let err = `Invalid param type for ${param.name}: Expected ${param.type} but got ${typeof requestParam}`;
                throw formatError(400, err);
            }
        }
    }
